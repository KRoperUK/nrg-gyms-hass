...
sys.path.append(
    os.path.join(os.path.dirname(__file__), "..", "custom_components", "nrg_gyms")
)
...
def main():
    ...
    if ident:
        print(
            f"Identity home club id: {ident.get('home_club_id')} default: {ident.get('default_club_id')}"
        )
    ...
    if prof:
        ...
        print(
            f"Email: {prof.get('email')} Phone: {prof.get('phone')} Referral: {prof.get('referral_code')}"
        )
