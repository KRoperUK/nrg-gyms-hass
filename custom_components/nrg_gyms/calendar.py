...
from .const import DATA_CONTRACTS_COORDINATOR, DATA_COORDINATOR, DOMAIN
...
class NrgBookingsCalendar(CalendarEntity):
    ...
    def event(self) -> CalendarEvent | None:
        ...
        try:
            _LOGGER.debug(
                "Creating CalendarEvent: summary=%s start=%s (tz=%s) end=%s (tz=%s)",
                item.get("summary"),
                start,
                start.tzinfo if start else None,
                end,
                end.tzinfo if end else None,
            )
            event = CalendarEvent(
            ...
            _LOGGER.error(
                "Failed to create CalendarEvent: %s (start: %s, end: %s)", e, start, end
            )
            return None
...
class NrgNextPaymentCalendar(CalendarEntity):
    ...
    def event(self) -> CalendarEvent | None:
        ...
        try:
            return CalendarEvent(
                summary=(f"{amount_fmt} due" if amount_fmt else None)
                or active.get("name")
                or "Membership Payment",
                start=start,
                end=end,
                description=f"Next payment for {active.get('name')}"
                + (f" ({amount_fmt})" if amount_fmt else ""),
    ...
    async def async_get_events(
        ...
        try:
            ...
            _LOGGER.error(
                "Failed to create payment CalendarEvent in async_get_events: %s", e
            )
            return []
