        ...
        if user_input is not None:
            ...
            if not ok:
                ...
                return self.async_create_entry(
                    title=f"NRG Gyms ({email})", data=user_input
                )
        ...
        data_schema = vol.Schema(
            {
                ...
                vol.Optional(CONF_USER_ID): selector.NumberSelector(
                    {"min": 1, "mode": "box"}
                ),
                vol.Optional(CONF_CLUB_ID): selector.NumberSelector(
                    {"min": 1, "mode": "box"}
                ),
                vol.Optional(
                    CONF_UPDATE_INTERVAL, default=DEFAULT_UPDATE_INTERVAL_SECONDS
                ): selector.NumberSelector({"min": 300, "mode": "box", "step": 60}),
            }
        )
